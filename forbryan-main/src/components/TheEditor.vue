...
onMounted(() => {
  ...
  g.batchUpdate(() => {
    ...
    const vertex02 = g.insertVertex(parent, null, 'a regular ellipse', 350, 90, 50, 50, <CellStyle>{
      shape: 'ellipse',
      fillColor: 'orange'
    })
...
<template>
  <div draggable="true" @drop="onDrop($event)">
    <div>
      <button @click="addSomething">Add Something</button>
    </div>
    <div ref="editorContainer" draggable="true" @drop="onDrop($event)"></div>
  </div>
</template>
