...
async def list_feedback(
    model_id: UUID, version_id: UUID, user: CurrentUser, skip: Skip = 0, limit: Limit = 20
) -> Page[FeedbackInfo]:
...
async def create_feedback(
    model_id: UUID, version_id: UUID, body: CreateFeedback, response: Response, user: CurrentUser
) -> Feedback:
