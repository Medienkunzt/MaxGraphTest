...
export class Editor extends EventSource {
  ...
  createGraph(): AbstractGraph {
    ...
    const popupMenuHandler =
      graph.getPlugin<PopupMenuHandler>("PopupMenuHandler");
    if (popupMenuHandler) {
  ...
  setStatusContainer(container: HTMLElement | null): void {
    if (!this.status && container) {
      ...
      this.addListener(InternalEvent.SAVE, () => {
        ...
        this.setStatus(
          `${
            translate(this.lastSavedResource) || this.lastSavedResource
          }: ${timestamp}`
        );
      });
  ...
  writeGraphModel(linefeed?: string): string {
    return new ModelXmlSerializer(this.graph.getDataModel()).export({
      pretty: false,
    });
  }
